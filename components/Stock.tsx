...
const Stock: React.FC<StockProps> = ({ pallets, packs }) => {
    ...
    const getStatusColor = (status: 'Creado' | 'En Proceso' | 'Expedido' | 'Disponible' | 'En Proceso' | 'Reservado') => {
        switch (status) {
            case 'Creado': return 'bg-blue-100 text-blue-800';
            case 'En Proceso': return 'bg-yellow-100 text-yellow-800';
            case 'Expedido': return 'bg-gray-100 text-gray-800';
            case 'Disponible': return 'bg-green-100 text-green-800';
            case 'Reservado': return 'bg-indigo-100 text-indigo-800';
            ...
            <Card>
                {activeTab === 'pallets' && (
                    <>
                                    ...
                                    {pallets.map((pallet) => (
                                        <tr key={pallet.id} className="hover:bg-gray-50">
                                            ...
                                            <td className="px-6 py-4 whitespace-nowrap text-sm">
                                                <span className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full ${getStatusColor(pallet.estado)}`}>
                                                    {pallet.estado}
                                                </span>
                                            </td>
                                        </tr>
                        ...
                        <div className="space-y-4 md:hidden">
                             {pallets.map((pallet) => (
                                <div key={pallet.id} className="bg-white p-4 rounded-lg shadow border border-gray-200">
                                    <div className="flex justify-between items-start">
                                        <div>
                                            <p className="text-sm font-bold text-dark-gray">{pallet.id}</p>
                                            <p className="text-xs text-gray-500">{getProductName(pallet.productId)}</p>
                                        </div>
                                        <span className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full ${getStatusColor(pallet.estado)}`}>
                                            {pallet.estado}
                                        </span>
                                    ...
                                    <div className="mt-3 text-sm text-gray-700 space-y-1">
                                        <p><span className="font-semibold">Lote:</span> {pallet.lote}</p>
                                        <p><span className="font-semibold">Fecha Entrada:</span> {pallet.fechaEntrada}</p>
                                        <p><span className="font-semibold">Total Botellas:</span> {(pallet.cajasPorPalet * pallet.botellasPorCaja).toLocaleString('es-ES')} ({pallet.cajasPorPalet} cajas)</p>
                                    </div>
                                </div>
                            ))}
                        </div>
                ...
                {activeTab === 'packs' && (
                    <>
                        ...
                        <div className="space-y-4 md:hidden">
                            {packs.map((pack) => (
                                <div key={pack.id} className="bg-white p-4 rounded-lg shadow border border-gray-200">
                                    <div className="flex justify-between items-start">
                                        <div>
                                            <p className="text-sm font-bold text-dark-gray">{pack.id}</p>
                                            <p className="text-xs text-gray-500">Pedido: {pack.pedidoCliente}</p>
                                        </div>
                                         <span className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full ${getStatusColor(pack.estado)}`}>
                                            {pack.estado}
                                        </span>
                                     ...
                                     <div className="mt-3 text-sm text-gray-700 space-y-1">
                                         <p><span className="font-semibold">Fecha:</span> {pack.fechaCreacion}</p>
                                    </div>
                                    <div className="mt-4 pt-3 border-t">
                                        <h5 className="text-xs font-bold uppercase text-gray-500 mb-2">Contenido</h5>
                                        {pack.productos.length > 0 ? (
                                            <ul className="space-y-1 text-sm">
                                            {pack.productos.map((producto, index) => (
                                                <li key={index}>
                                                    <span className="font-semibold">{getProductName(producto.productId)}</span> ({producto.cantidad.toLocaleString('es-ES')} botellas) - Lote: {producto.lote}
                                                </li>
                                            ))}
                                            </ul>
                                        ) : (
                                            <p className="text-sm text-gray-500 italic">Sin productos detallados.</p>
                                        )}
                                    </div>
                                </div>
                            ))}
